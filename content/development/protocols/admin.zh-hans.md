...
读取或写入 CTAP 应用的 SM2 配置。配置为 8 字节，依次为 32 位的 `curve_id` 和 32 位的 `algo_id`（COSE 标识符）。写入时 `algo_id` 不得与 ES256 / EdDSA / ML-DSA-65 的算法标识冲突，否则返回 `6A80`。
...
读取或设置 NFC 是否启用。该指令由具体硬件平台实现。设置时需要先验证 PIN。
...
| CLA   | 00h   |
| INS   | 31h   |
| P1    | 00h 表示固件版本，01h 表示硬件型号，02h 表示 canokey-core 提交哈希 |
| P2    | 00h   |
| Le    | 00h   |
...
读取或配置触摸输出（Pass）应用。Pass 有两个槽位：短按和长按。OATH 类型的槽位由 OATH 应用设置，不能通过 Write Pass Config 写入。
...
管理键盘输出使用的键盘布局表。布局表固定为 128 项，第 N 项将 ASCII 码 N 映射为 2 字节 `{修饰键, HID usage}`。usage 为 0 表示跳过该字符。写入的布局表生效后将取代内置的 QWERTY 布局。
